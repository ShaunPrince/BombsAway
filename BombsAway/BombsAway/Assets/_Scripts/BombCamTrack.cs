...
public class BombCamTrack : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        float theta2 = Vector3.Angle(-this.transform.parent.up *100000, hit.point + this.transform.parent.TransformVector(new Vector3(0, 0, -bap.radius)));
        ...
        float x1 = Vector3.Magnitude(new Vector3(this.transform.parent.position.x,hit.point.y,this.transform.parent.position.z)
                    - hit.point);
        float x2 = x1 - bap.radius;
        float y1 = this.transform.position.y - hit.point.y;
        theta1 = Mathf.Rad2Deg * Mathf.Atan(x1 / y1);
        theta2 = Mathf.Rad2Deg * Mathf.Atan(x2 / y1);
        float y2 = 1/(Mathf.Tan(Mathf.Deg2Rad*theta1) - Mathf.Tan(Mathf.Deg2Rad*theta2));
        Debug.Log(hit.point);
        Debug.Log("Radius: " + bap.radius);
        Debug.Log("x1: " + x1);
        ...
        cylinder.transform.localPosition = new Vector3(0, -y2,(Mathf.Tan(Mathf.Deg2Rad*theta2)*y2)+1);
